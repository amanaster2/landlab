...
class KinwaveImplicitOverlandFlowADM(Component):
    """
    ...
    Construction:
        ...
        KinwaveImplicitOverlandFlowADM(grid, precip_rate=1.0,
                                    precip_duration=1.0,
                                    infilt_rate=0.0,
                                    roughness=0.01, **kwds)
    ...
    >>> kw = KinwaveImplicitOverlandFlowADM(rg)
    ...
    _name = 'KinwaveImplicitOverlandFlowADM'
...
#            'flow discharge component in the direction of the link',
    ...
    def __init__(self, grid, runoff_rate=1.0,
		...
		# Get roughness field
        try:
            self.vel_coef = 1/grid.at_node['roughness']
        except:
            raise
...
#                                        'surface_water__specific_discharge')
    ...
    def run_one_step(self, dt, current_time=0.0, runoff_rate=None, **kwds):
        ...
        if self.changing_topo or self.first_iteration:
            ...
            self.alpha[cores] = (
                self.vel_coef[cores] * self.grad_width_sum[cores] * dt 
        ...
        for i in range(len(self.nodes_ordered) - 1, -1, -1):
            ...
            if self.grid.status_at_node[n] == 0:
                ...
                Heff = (self.weight * self.depth[n]
                        + (1.0 - self.weight) * cc)
                outflow = (self.vel_coef[n] * (Heff ** self.depth_exp)
